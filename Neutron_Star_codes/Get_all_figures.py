...
if verify_files(matrices_list):
    ...
    print(f'In GR, the relative variation of hbar for the most massive NS of {mass_a_GR[index_max_M_GR]:.1f} Solar mass and a radius of {size_a_GR[index_max_M_GR]:.1f} km is {delta_hbar_a_GR[index_max_M_GR] * 1e2:.1f}% with an associated density of {den_space[index_max_M_GR]:.1f} MeV/fm3')
    ...
    print(f'In ER with retroaction, above the density of {dens_tresh_ER_retro:.0f} Mev/fm^3, the speed of sound somewhere inside the neutron star becomes larger than the conservative limit c/sqrt(3)\n')
    ...
    print('The maximum ADM mass and the corresponding maximum density for GR, ER, and ER with retroaction are given by : ')
    print( f'm_GR = {Max_mass_ADM_GR:.1f} Solar mass \ndensity_GR = {Max_density_ADM_GR:.1f} Mev/fm3')
    print(f'm_ER = {Max_mass_ADM:.1f} Solar mass \ndensity_ER = {Max_density_ADM:.1f} Mev/fm3')
    print(f'm_ER_retro = {Max_mass_ADM_ER_retro:.1f} Solar mass \ndensity_ER_retro = {Max_density_ADM_ER_retro:.1f} Mev/fm3\n')
...
##################################################################
    ...
    for dependence in tqdm(Dependence):
        if dependence == -3:
            ...
            np.save(f'./save_hbar_NS/matrice_star_m3.npy',hbar_star_m3)
            ...
            np.save(f'./save_hbar_NS/matrice_star_m2.npy',hbar_star_m2)
            ...
            np.save(f'./save_hbar_NS/matrice_star_m1.npy',hbar_star_m1)
            ...
            np.save(f'./save_hbar_NS/matrice_star_0.npy',hbar_star)
            ...
            np.save(f'./save_hbar_NS/matrice_star_1.npy',hbar_star_1)
            ...
            np.save(f'./save_hbar_NS/matrice_star_2.npy',hbar_star_2)
            ...
            np.save(f'./save_hbar_NS/matrice_star_3.npy',hbar_star_3)
